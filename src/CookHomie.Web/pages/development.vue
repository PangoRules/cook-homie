  ...
  <div class="p-6">
    ...
    <div v-if="activeTab === 'inventory'">
      <div class="flex gap-2 mb-5 flex-wrap">
        ...
        <SharedButton
          variant="secondary"
          :disabled="inventoryPolling"
          @click="startInventoryPolling"
          >Start Polling</SharedButton
        >
        <SharedButton
          variant="secondary"
          :disabled="!inventoryPolling"
          @click="stopInventoryPolling"
          >Stop Polling</SharedButton
        >
      </div>
      ...
      <div v-else>
        ...
        <ul class="list-none p-0 flex flex-col gap-1 mt-3">
          ...
          >
            ...
            <span v-if="item.expiresAt" class="text-text-muted">
              - Expires: {{ formatDate(item.expiresAt) }}</span
            >
          </li>
        </ul>
      </div>
    ...
    <div v-if="activeTab === 'polling'" class="max-w-2xl">
      <div class="mb-6">
        ...
        <div class="flex gap-2">
          <SharedButton :disabled="pollingActive" @click="startPollingData"
            >Start Polling</SharedButton
          >
          <SharedButton variant="secondary" :disabled="!pollingActive" @click="stopPollingData"
            >Stop Polling</SharedButton
          >
        </div>
      </div>
      ...
      <div>
        ...
        />
        <div
          v-else-if="pollingData"
          class="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4 mb-5"
        >
          <div
            v-for="[label, value] in [
              ['Timestamp', formatTimestamp(pollingData.timestamp)],
              ['Items Count', pollingData.itemsCount],
              ['Last polled', formatTimestamp(pollingData.lastPolled)],
            ]"
            :key="label"
            class="flex flex-col p-3 bg-surface-hover rounded-md"
          >
        ...
        <div class="mt-5 p-4 bg-surface-hover rounded-md max-h-52 overflow-y-auto">
          ...
          <div
            ...
            :key="index"
            class="flex justify-between py-1 border-b border-border text-sm last:border-0"
          >
...
<script setup lang="ts">
  ...
  const pollingData = ref<{ timestamp: number; itemsCount: number; lastPolled: number } | null>(
    null
  );
  ...
  const pollingFetch = usePollingFetch<{
    timestamp: number;
    itemsCount: number;
    lastPolled: number;
  }>("/api/polling-data", {
  ...
  function refreshPollingData() {
    pollingFetch.refresh().then(() => addPollingLog("Manual refresh performed"));
  }
</script>
