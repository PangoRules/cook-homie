...
vi.mock("../composables/useLocale", () => ({
  useLocale: () => ({
    locale: { value: "en-US" },
    setLocale: () => {},
    formatNumber: (v: number | string) => (typeof v === "number" ? v.toLocaleString("en-US") : v),
  }),
}));
...
describe("DashboardStatCard", () => {
  ...
  it("renders label and value", () => {
    const wrapper = mount(DashboardStatCard, {
      ...mountOptions,
      props: { label: "Expiring", value: 3, loading: false },
    });
  ...
  it("formats number values with locale", () => {
    const wrapper = mount(DashboardStatCard, {
      ...mountOptions,
      props: { label: "Items", value: 1234, loading: false },
    });
  ...
  it("applies warning variant class", () => {
    const wrapper = mount(DashboardStatCard, {
      ...mountOptions,
      props: { label: "Warn", value: 5, variant: "warning", loading: false },
    });
...
describe("DashboardPanel", () => {
  ...
  it("shows refresh button when showRefresh is true", () => {
    const wrapper = mount(DashboardPanel, {
      ...mountOptions,
      props: { title: "P", showRefresh: true, loading: false, hasData: true },
    });
