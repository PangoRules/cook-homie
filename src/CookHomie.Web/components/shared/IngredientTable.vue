      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-border">
            ...
            <th v-if="mode === 'readonly'" class="text-center text-text-muted font-medium w-[80px]">
              Status
            </th>
              ...
              <td colspan="5" class="py-3">
                <div class="rounded-lg border border-border bg-background-muted p-3">
                  ...
                  <div class="mt-3 flex justify-end gap-2">
                    <button type="button" class="btn btn-secondary text-xs" @click="cancelEditing">
                      Cancel
                    </button>
                    <button
                      type="button"
                      class="btn btn-primary text-xs"
                      @click="finishEditing(row.index)"
                    >
                      Done
                    </button>
                  </div>
              ...
              <td v-if="mode === 'editable'" class="py-2 text-right">
                <div class="flex justify-end gap-2">
                  <button
                    ...
                    @click="editIngredient(row.index)"
                  >
                    Edit
                  </button>
                  <button
...
<script setup lang="ts">
  ...
  const initialEditingIndex =
    props.startEditingFirstRow && props.mode === "editable" && props.ingredients.length > 0
      ? 0
      : null;
  const editingIndex = ref<number | null>(initialEditingIndex);
  const draftIngredient = ref<DraftIngredient | null>(
    initialEditingIndex === null ? null : cloneIngredient(props.ingredients[initialEditingIndex])
  );
  const editError = ref("");
  const ingredientNameInputRef = ref<HTMLInputElement | readonly HTMLInputElement[]>();
  const totalPages = computed(() =>
    Math.max(1, Math.ceil(props.ingredients.length / props.pageSize))
  );
  ...
  const cancelEditing = () => {
    ...
    const ingredient =
      index !== null && index < props.ingredients.length ? props.ingredients[index] : undefined;
